...
// ─── Page (client) ─────────────────────────────────────────────────────────────
// Reçoit les 4 listes initiales pré-chargées en SSR (cf. ./page.tsx).
// Tous les setters sont conservés : un rafraîchissement après mutation
// (envoi WhatsApp → fetchAll()) garde le comportement existant.
export default function WhatsAppCenterClient({
  initialSeances,
  initialRdvs,
  initialFeedbacks,
  initialReadySeances,
  ...
  const [rdvs, setRdvs] = useState<any[]>(initialRdvs)
  const [feedbacks, setFeedbacks] = useState<any[]>(initialFeedbacks)
  const [readySeances, setReadySeances] = useState<any[]>(initialReadySeances)
  ...
  // Check URL param on mount to open correct tab — pas de fetchAll au 1er
  // rendu, les 4 listes sont déjà fournies en SSR via les props initial*.
  useEffect(() => {
