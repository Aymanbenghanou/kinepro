...
import { Plus, X, Pencil, Trash2, Globe, User } from 'lucide-react'
      ...
      <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 12, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '14px 20px', borderBottom: '1px solid #E2E8F0', background: '#F8FAFC', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: '#374151' }}>
              {tab === 'global' ? `${list.length} type${list.length > 1 ? 's' : ''} global${list.length > 1 ? 'aux' : ''}` : `${list.length} type${list.length > 1 ? 's' : ''} personnel${list.length > 1 ? 's' : ''}`}
            </span>
          </div>
          <button
            onClick={() => openAdd(tab)}
            style={{ display: 'flex', alignItems: 'center', gap: 6, background: tab === 'global' ? '#2563EB' : '#7C3AED', color: 'white', border: 'none', borderRadius: 7, padding: '7px 14px', cursor: 'pointer', fontWeight: 600, fontSize: 13 }}
          >
        ...
        {list.length === 0 ? (
          <div style={{ padding: '40px 20px', textAlign: 'center', color: '#94A3B8', fontSize: 14 }}>
            {tab === 'global' ? 'Aucun type global défini.' : 'Aucun type personnel. Cliquez sur Ajouter.'}
          </div>
        ) : (
    ...
    <div>
      ...
      <div style={{ padding: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 24, background: '#F1F5F9', padding: 4, borderRadius: 10, width: 'fit-content' }}>
          {([
            { key: 'global'   as Tab, label: '🌐 Types globaux',     desc: 'Partagés avec tous' },
            { key: 'personal' as Tab, label: '👤 Types personnels',   desc: 'Par kiné' },
          ]).map(tab => (
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: 'white', borderRadius: 16, padding: 28, width: 500, boxShadow: '0 20px 60px rgba(0,0,0,0.18)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  {COULEURS.map(c => (
                    <button key={c.hex} type="button"
                      ...
                      style={{
                        width: 34, height: 34, borderRadius: 8, background: c.hex,
                        border: form.couleur === c.hex ? '3px solid #0F172A' : '2px solid white',
                        cursor: 'pointer',
                        boxShadow: form.couleur === c.hex ? `0 0 0 2px ${c.hex}40` : '0 1px 3px rgba(0,0,0,0.15)',
                      }} />
                  ))}
                </div>
                <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ width: 20, height: 20, borderRadius: 5, background: form.couleur }} />
                  <span style={{ fontSize: 12, color: '#64748B', fontFamily: 'monospace' }}>{form.couleur}</span>
                </div>
              </div>

              {/* Praticien (personal type) */}
              <div>
                ...
                >
                  <option value="">Global (partagé avec tous)</option>
                  {praticiens.map((p: any) => (
                    <option key={p.id} value={p.id}>Personnel — Dr. {p.prenom} {p.nom}</option>
                  ))}
                </select>
                <p style={{ fontSize: 12, color: '#64748B', marginTop: 4 }}>
                  Laissez sur « Global » pour que ce type soit disponible pour tous les praticiens.
                </p>
              </div>
              ...
              {form.nom && (
                <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 10, padding: 12 }}>
                  <p style={{ fontSize: 12, color: '#64748B', margin: '0 0 8px', fontWeight: 600 }}>Aperçu dans le calendrier :</p>
                  <div style={{
                    background: form.couleur, color: 'white', borderRadius: 6,
                    padding: '6px 10px', fontSize: 12, fontWeight: 600, display: 'inline-block',
                  }}>
                    {form.nom} · {form.dureeDefaut}min
                  </div>
                </div>
              )}
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
                ...
                <button onClick={handleSave} disabled={saving}
                  style={{ flex: 1, padding: '10px', border: 'none', borderRadius: 8, background: saving ? '#93C5FD' : '#2563EB', color: 'white', cursor: saving ? 'not-allowed' : 'pointer', fontWeight: 600 }}>
      ...
      {deleteTarget && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
          <div style={{ background: 'white', borderRadius: 16, padding: 28, width: 420, boxShadow: '0 20px 60px rgba(0,0,0,0.18)' }}>
            <div style={{ textAlign: 'center', marginBottom: 20 }}>
              ...
              <p style={{ fontSize: 14, color: '#64748B', margin: 0 }}>
                <strong style={{ color: '#0F172A' }}>« {deleteTarget.nom} »</strong> sera supprimé.
                <br />
                <span style={{ fontSize: 13, color: '#94A3B8' }}>
                  Les séances et RDV existants ne seront pas affectés.
                </span>
              </p>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => handleDelete(deleteTarget)}
                style={{ flex: 1, padding: '10px', border: 'none', borderRadius: 8, background: '#DC2626', color: 'white', cursor: 'pointer', fontWeight: 600 }}>
                Supprimer
              </button>
            </div>
          </div>
