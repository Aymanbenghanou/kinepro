...
export default function MedicalTemplate({ data }: { data: SiteData }) {
  ...
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'} style={{ fontFamily: isRTL ? "'Cairo', sans-serif" : "'Segoe UI', system-ui, sans-serif", margin: 0, padding: 0, background: '#FFFFFF', color: '#1a1a1a' }}>
      ...
      {/* SERVICES */}
      <section id="services" style={{ padding: '90px 5%', background: '#fff' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div className="services-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 28 }}>
            {seanceTypes.length > 0 ? seanceTypes.map(s => (
              <div key={s.id} className="service-card fade-in" style={{ borderLeftColor: s.couleur || accent }}>
                <h3 style={{ fontSize: 18, fontWeight: 700, color: primary, marginBottom: 10 }}>{s.nom}</h3>
                {s.description && <p style={{ fontSize: 14, color: '#64748B', lineHeight: 1.7, marginBottom: 14 }}>{s.description}</p>}
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 13, color: accent, fontWeight: 600 }}>⏱ {s.dureeDefaut} min</span>
                  {s.tarifDefaut && <span style={{ fontSize: 13, color: '#10B981', fontWeight: 600 }}>💰 {s.tarifDefaut} DH</span>}
                </div>
              </div>
            )) : (
              ['Kinésithérapie', 'Rééducation', 'Ostéopathie'].map(name => (
                <div key={name} className="service-card fade-in">
                  <h3 style={{ fontSize: 18, fontWeight: 700, color: primary, marginBottom: 10 }}>{name}</h3>
                  <p style={{ fontSize: 14, color: '#64748B', lineHeight: 1.7 }}>{lang === 'ar' ? 'رعاية متخصصة ومخصصة لاحتياجاتك' : 'Soins spécialisés adaptés à vos besoins.'}</p>
                </div>
              ))
            )}
          </div>
          ...
          <div className="contact-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'start' }}>
            <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              {cabinet.whatsappNumber && (
                <a href={`https://wa.me/${cabinet.whatsappNumber}`} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: '#25D366', color: '#fff', padding: '13px 28px', borderRadius: 10, textDecoration: 'none', fontWeight: 700, fontSize: 15, width: 'fit-content', boxShadow: '0 4px 16px rgba(37,211,102,0.3)' }}>
