    ...
    <div dir={isRTL ? 'rtl' : 'ltr'} style={{ fontFamily: isRTL ? "'Cairo', sans-serif" : "'Segoe UI', system-ui, sans-serif", margin: 0, padding: 0, background: '#FFFFFF', color: '#1a1a1a' }}>
      ...
      <section id="services" style={{ padding: '96px 5%', background: '#F8FAFC' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div className="services-grid-p" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
            {seanceTypes.length > 0 ? seanceTypes.map(s => (
              <div key={s.id} className="service-card-p fade-in" style={{ borderTopColor: s.couleur || accent }}>
                <div style={{ width: 52, height: 52, borderRadius: '50%', background: `linear-gradient(135deg, ${s.couleur || accent}, #D97706)`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 18, boxShadow: `0 4px 16px ${accent}40`, transition: 'transform 0.2s' }} className="service-icon-circle-p">
                  <ServiceIcon nom={s.nom} size={26} color="white" />
                </div>
                <h3 style={{ fontSize: 17, fontWeight: 800, color: primary, marginBottom: 8 }}>{s.nom}</h3>
                {s.description && <p style={{ fontSize: 13, color: '#64748B', lineHeight: 1.65, marginBottom: 12 }}>{s.description}</p>}
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
                  <span style={{ fontSize: 12, color: accent, fontWeight: 700, background: `rgba(245,158,11,0.1)`, padding: '3px 10px', borderRadius: 999 }}>⏱ {s.dureeDefaut} min</span>
                ...
                <a href="#booking" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, color: accent, fontSize: 13, fontWeight: 700, textDecoration: 'none' }}>
              ...
              ['Kinésithérapie', 'Rééducation', 'Ostéopathie'].map(name => (
                <div key={name} className="service-card-p fade-in">
                  <div style={{ width: 52, height: 52, borderRadius: '50%', background: accent, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 18 }}>
                    <ServiceIcon nom={name} size={26} color="white" />
                  </div>
                  <h3 style={{ fontSize: 17, fontWeight: 800, color: primary, marginBottom: 8 }}>{name}</h3>
                  <p style={{ fontSize: 13, color: '#64748B', lineHeight: 1.65 }}>{lang === 'ar' ? 'علاج متخصص بأعلى المعايير' : 'Soins spécialisés selon les meilleures pratiques.'}</p>
                </div>
              ))
            )}
          </div>
