...
- **Supabase Free tier auto-pauses projects after ~7 days of inactivity.** The wake-up is a cold start that can exceed the pool's `connectionTimeoutMillis`. Symptom: public routes throw a connectivity error that used to be swallowed and rendered as "dossier introuvable" on `/patient-public/[token]` / `/scan/[token]`. Fix in place: 15 s connection timeout + `isDbConnectivityError()` helper (`src/lib/db-errors.ts`) + 503 `service_unavailable` mapped to a "réessayez dans quelques instants" screen. **Real production ⇒ upgrade to a paid Supabase plan** — a keep-alive cron (e.g. daily-summary) reduces but does not eliminate the risk, and Free tier is not intended for production apps with real patient data.
